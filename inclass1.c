...
int main(int argc, char** argv){
	...
	*/
	printf("#####student code start#####\n");
    //////////Your code here//////////
	printf("Signed_Int + 1 : %d\n", INT_MAX + 1);
	printf("Unsigned_Int + 1 : %u\n",(unsigned int) UINT_MAX + 1);
	printf("Signed_Long + 1 : %ld\n",(long) LONG_MAX +  1);
	printf("Float + 1 : %g\n",(float) FLT_MAX + 1);
	printf("Float + 1000 : %g\n",(float) FLT_MAX + 1000); 
	//////////End of the code/////////
	printf("#####student code end#####\n");
	...
	Answer:
	Reason of the result is A char* is just a pointer to some 
	memory, and has a size of whatever a pointer is on your architecture
	8 bytes --> 64-bit architecture;4 bytes -->32-bit architecture
	*/
